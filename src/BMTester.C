...
int BMTester::test1()
{
  ...
  for (int i = first; i <= last; i++)
  {
    ...
    if(TESTDEBUG)MINIBASE_BM->print();
...
int BMTester::test2()
{
  ...
  cout << endl
       << "Testing replacement policy with respect to hated pages..." << endl;
  ...
  cout << endl
       << "Testing replacement policy with respect to loved pages..." << endl;
...
int BMTester::test5()
{
  ...
  if (MINIBASE_BM->unpinPage(pages[0]) != OK)
  {
    ...
    cout << "\tUnpinning of a pinned page successful\n";
  ...
  if (MINIBASE_BM->unpinPage(pages[0]) == OK)
  {
    ...
    cout << "\tUnpinning of an unpinned page failed\n";
  ...
  if (MINIBASE_BM->pinPage(999, tmpPage) == OK)
  {
    ...
    cout << "\tPinning of a non-existent page failed\n";
  ...
  if (MINIBASE_BM->unpinPage(999) == OK)
  {
    ...
    cout << "\tUnpinning of a non-existent page failed\n";
  ...
  if (MINIBASE_BM->freePage(pages[0]) == OK)
  {
    ...
    cout << "\tFreeing a pinned page failed \n";
  ...
  for (i = 0; i < NUMBUF; i++)
  {
    if (MINIBASE_BM->unpinPage(pages2[i]) != OK)
      st = FAIL;
    if (MINIBASE_BM->freePage(pages2[i]) != OK)
      st = FAIL;
  }
  if (st == FAIL)
  {
